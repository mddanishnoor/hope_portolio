...
class HoldMeButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTapDown: (details) =>
          context.read<CursorProvider>().toggleMagnify(true, fullScreen: true),
      onTapUp: (details) => context
          .read<CursorProvider>()
          .toggleMagnify(false, fullScreen: false),
      // onTap: () {
      //   context.read<CursorProvider>().toggleMagnify(
      //       !context.read<CursorProvider>().isMagnified,
      //       fullScreen: !context.read<CursorProvider>().isMagnified);
      // },
      child: Container(
        width: 54,
        height: 52,
